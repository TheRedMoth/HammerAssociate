...
// Функция для установки значения в реестре
BOOL SetRegistryValue(HKEY hKey, const char* subKey, const char* valueName, DWORD valueType, const void* valueData, DWORD dataSize) {
    HKEY hSubKey;
    if (RegCreateKeyEx(hKey, subKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hSubKey, NULL) != ERROR_SUCCESS) {
        return FALSE;
    }

    if (RegSetValueEx(hSubKey, valueName, 0, valueType, (const BYTE*)valueData, dataSize) != ERROR_SUCCESS) {
        RegCloseKey(hSubKey);
        return FALSE;
    }
...
void OpenHammer(const char* filePath) {
    ...
    if (hammerPath) {
        ...
        DWORD value = 131072;
        SetRegistryValue(HKEY_CURRENT_USER, "SOFTWARE\\Valve\\Hammer\\3D Views", "BackPlane", REG_DWORD, &value, sizeof(value));
        SetRegistryValue(HKEY_CURRENT_USER, "SOFTWARE\\Valve\\Hammer\\3D Views", "DetailDistance", REG_DWORD, &value, sizeof(value));
        SetRegistryValue(HKEY_CURRENT_USER, "SOFTWARE\\Valve\\Hammer\\3D Views", "ModelDistance", REG_DWORD, &value, sizeof(value));
        SetRegistryValue(HKEY_CURRENT_USER, "SOFTWARE\\Valve\\Hammer\\General", "Undo Levels", REG_SZ, "131072", strlen("131072") + 1);
